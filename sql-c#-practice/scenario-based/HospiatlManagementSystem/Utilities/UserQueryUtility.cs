...
public async Task<(int UserId, string Role, byte[] Hash, byte[] Salt)> GetLoginDataAsync(string username)
{
    var conn = await _db.OpenAsync();
    using SqlCommand cmd = new SqlCommand("sp_ValidateLogin", conn);
    cmd.CommandType = CommandType.StoredProcedure;
...
public async Task GetMedicalVisitsRawAsync()
{
...
public async Task SetSessionContextAsync(int userId , string role)
{
    var conn = await _db.OpenAsync();
